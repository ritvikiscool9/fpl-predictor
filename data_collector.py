    ...
    try:
        ...
        for record in gameweek_data:
            db_record = {
                ...
                "selected": record.get(
                    "selected", record.get("selected_by_percent", 0)
                ),
        ...
        for i in range(0, len(db_records), batch_size):
            ...
            result = (
                ...
                .upsert(batch, on_conflict="season,gameweek,fpl_player_id")
                .execute()
            )
...
def save_fixtures_data_to_db(fixtures_data):
    ...
    try:
        ...
        for i in range(0, len(db_records), batch_size):
            ...
            result = (
                supabase.table("historical_fixtures")
                .upsert(batch, on_conflict="season,fpl_fixture_id")
                .execute()
            )
...
def validate_collected_data():
    ...
    try:
        ...
        print(
            f"   Player Stats: {player_stats_count:,} / {expected_stats:,} expected ({(player_stats_count/expected_stats*100):.1f}%)"
        )
        print(
            f"   Fixtures: {fixtures_count:,} / {expected_fixtures:,} expected ({(fixtures_count/expected_fixtures*100):.1f}%)"
        )
...
def analyze_data_quality():
    ...
    try:
        result = (
            supabase.table("historical_player_stats").select("*").limit(1000).execute()
        )
